...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    const animate = (time) => {
      ...
      if (gameState === 'playing') {
        ...
        if (jackpotTimer > 30000) { // Every 30s check
  ...
  const updateMyCells = (deltaTime) => {
    ...
    myPlayerCellsRef.current.forEach(cell => {
      ...
      // Merge / Push physics
      myPlayerCellsRef.current.forEach(other => {
        ...
        if (d < rSum) {
          ...
          const push = (rSum - d) / 2; // Rigid body push
          cell.x += Math.cos(angle) * push * 0.1; // Soft push
    ...
    myPlayerCellsRef.current.forEach((cell) => {
      ...
      for (let i = foodRef.current.length - 1; i >= 0; i--) {
        ...
        if (Math.hypot(cell.x - f.x, cell.y - f.y) < cell.radius) {
          ...
          // Rewards
          if (isJackpot) {
            cell.radius += 20; // Massive growth
            showNotification("💰 YOU GOT THE JACKPOT! 💰");
          } else {
            cell.radius += 0.5;
          }

          // Respawn normal food immediately, Jackpot rarely
